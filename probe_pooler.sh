...
for (( p=1; p<=$pool_length; p++ ))
   ...
   for (( c=1; c<=$temp_acc_length; c++ ))
	...
	cp ~/probe_pooler/.probes/$temp_acc_search_term/*/*all*/csv/*$temp_combo_search_term* ~/probe_pooler/.tmp/"subpool_000$p"/$temp_combo_search_term
   ...
   echo "$session_record_num" > ~/probe_pooler/.tmp/"subpool_000$p"/session_record.txt
   mv ~/probe_pooler/.tmp/"subpool_000$p" ~/probe_pooler/.all_probe_pools/"$pool_id" 
